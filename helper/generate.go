...
func DecryptAES256CBC(data string) (result string, err error) {
	defer func() {
		if r := recover(); r != nil {
	...
	key := HashSha256(config.AppConfig.Hashing.Key) // 32 bytes for AES-256
...
func EncryptAES256CBC(data []byte) (string, error) {

	private_key := HashSha256(config.AppConfig.Hashing.Key)
	data = pad(data)
